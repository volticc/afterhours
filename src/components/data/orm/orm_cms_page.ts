...
// - Entity ID: 102019afbb8d7bd7cfd8a9166c9f1eb5b55.
...
/**
 * ORM class for CmsPage entity.
 * It uses Singleton pattern to prevent re-initialization.
 */
export class CmsPageORM {
  private static instance: CmsPageORM | null = null;
  ...
  private constructor() {
    ...
    this.entityId = '102019afbb8d7bd7cfd8a9166c9f1eb5b55';
    this.entityName = 'cms_page';
  ...
  /**
   * Get singleton instance of CmsPageORM
   */
  public static getInstance(): CmsPageORM {
    if (!CmsPageORM.instance) {
      CmsPageORM.instance = new CmsPageORM();
    }
    
    return CmsPageORM.instance;
  }

  /**
   * Get all CmsPage records
   * This returns all data without pagination.
   * To query data with page and filter conditions, use `list*()` instead.
   */
  async getAllCmsPage(): Promise<CmsPageModel[]> {
  ...
  /**
   * Insert (create) new CmsPage record(s)
   ...
   */
  async insertCmsPage(data: CmsPageModel[]): Promise<CmsPageModel[]> {
    const structured = data.map((item) => CreateData(CmsPageModelToValues(item)));
  ...
  /**
   * Purge all CmsPage records
   */
  async purgeAllCmsPage(): Promise<void> {
  ...
  /**
   * List CmsPage records with filters
   * This function provides search capabilities like filtering, sorting, pagination.
   */
  async listCmsPage(filter?: Filter, sort?: Sort, paginate?: Page): Promise<[CmsPageModel[], Page]> {
  ...
  /**
   * Get cms_page by Id index
   * This function gets data by index.
   */
  async getCmsPageById(
    id: string,
  ...
  /**
   * Set (update) cms_page by Id index
  ...
  async setCmsPageById(
    ...
    const values = CmsPageModelToValues(data);
  ...
  /**
   * Delete cms_page by Id index
   */
  async deleteCmsPageById(
    id: string
  ): Promise<void> {
    const index = createIndexId(
      id
  ...
  /**
   * Get cms_page by Slug index
   * This function gets data by index.
   */
  async getCmsPageBySlug(
    slug: string,
  ): Promise<CmsPageModel[]> {
    const index = createIndexSlug(
      slug
    );
  ...
  /**
   * Set (update) cms_page by Slug index
   * This function replaces data, so the data must be complete.
   * Must keep `id`, `data_creator`, `create_time` unchanged as original data (as fetched).
   * DO NOT SET `data_updater` and `update_time` since backend will fill it automatically.
   * It will respond the set record, with `data_updater` and `update_time` filled by backend.
   */
  async setCmsPageBySlug(
    slug: string,
    data: CmsPageModel
  ): Promise<CmsPageModel[]> {
    const index = createIndexSlug(
  ...
  /**
   * Delete cms_page by Slug index
   */
  async deleteCmsPageBySlug(
    slug: string
  ): Promise<void> {
    const index = createIndexSlug(
      slug
  ...
  /**
   * Get cms_page by Status index
   * This function gets data by index.
   */
  async getCmsPageByStatus(
    status: CmsPageStatus,
  ): Promise<CmsPageModel[]> {
  ...
  /**
   * Set (update) cms_page by Status index
   * This function replaces data, so the data must be complete.
   * Must keep `id`, `data_creator`, `create_time` unchanged as original data (as fetched).
   * DO NOT SET `data_updater` and `update_time` since backend will fill it automatically.
   * It will respond the set record, with `data_updater` and `update_time` filled by backend.
   */
  async setCmsPageByStatus(
    status: CmsPageStatus,
    data: CmsPageModel
  ): Promise<CmsPageModel[]> {
    const index = createIndexStatus(
  ...
  /**
   * Delete cms_page by Status index
   */
  async deleteCmsPageByStatus(
    status: CmsPageStatus
  ): Promise<void> {
    const index = createIndexStatus(
      status
  ...
  /**
   * Get cms_page by DataCreator index
   * This function gets data by index.
   */
  async getCmsPageByDataCreator(
    data_creator: string,
  ): Promise<CmsPageModel[]> {
  ...
  /**
   * Set (update) cms_page by DataCreator index
   * This function replaces data, so the data must be complete.
   * Must keep `id`, `data_creator`, `create_time` unchanged as original data (as fetched).
   * DO NOT SET `data_updater` and `update_time` since backend will fill it automatically.
   * It will respond the set record, with `data_updater` and `update_time` filled by backend.
   */
  async setCmsPageByDataCreator(
    data_creator: string,
    data: CmsPageModel
  ): Promise<CmsPageModel[]> {
    const index = createIndexDataCreator(
  ...
  /**
   * Delete cms_page by DataCreator index
   */
  async deleteCmsPageByDataCreator(
    data_creator: string
  ): Promise<void> {
    const index = createIndexDataCreator(
      data_creator
  ...
  /**
   * Get cms_page by DataUpdater index
   * This function gets data by index.
   */
  async getCmsPageByDataUpdater(
    data_updater: string,
  ): Promise<CmsPageModel[]> {
  ...
  /**
   * Set (update) cms_page by DataUpdater index
   * This function replaces data, so the data must be complete.
   * Must keep `id`, `data_creator`, `create_time` unchanged as original data (as fetched).
   * DO NOT SET `data_updater` and `update_time` since backend will fill it automatically.
   * It will respond the set record, with `data_updater` and `update_time` filled by backend.
   */
  async setCmsPageByDataUpdater(
    data_updater: string,
    data: CmsPageModel
  ): Promise<CmsPageModel[]> {
    const index = createIndexDataUpdater(
  ...
  /**
   * Delete cms_page by DataUpdater index
   */
  async deleteCmsPageByDataUpdater(
    data_updater: string
  ): Promise<void> {
    const index = createIndexDataUpdater(
      data_updater
  ...
  /**
   * Convert result data to CmsPageModel data array
   */
  private resultToData(values: Data[]): CmsPageModel[] {
    return values.map((item: Data) => {
      if (item.structured && item.structured.length > 0) {
        return CmsPageModelFromValues(item.structured);
      }

      if (item.serialized) {
        try {
          const parsedData = JSON.parse(item.serialized) as CmsPageModel;
          return parsedData;
        } catch (error) {
          console.error('Error parsing serialized CmsPageModel data: ', error, item.serialized);
          return null;
        }
      }

      return null;
    }).filter((item): item is CmsPageModel => item !== null);
  }
}

/**
 * Convert CmsPageModel data to Value array
 */
function CmsPageModelToValues(data: CmsPageModel): Value[] {
  const fieldMappings = [
    ...
    { key: 'slug', type: DataType.string, defaultValue: '' },
    { key: 'status', type: DataType.enumeration, defaultValue: 0 },
    { key: 'template_layout', type: DataType.string, defaultValue: '' },
    { key: 'seo_title', type: DataType.string, defaultValue: null },
    { key: 'seo_description', type: DataType.string, defaultValue: null },
  ];

  return fieldMappings.map(({ key, type, defaultValue }) => {
    const value = data[key as keyof CmsPageModel] ?? defaultValue;
    return CreateValue(type, value, key);
  });
}

/**
 * Convert Value array to CmsPageModel data
 */
function CmsPageModelFromValues(values: Value[]): CmsPageModel {
  const data: Partial<CmsPageModel> = {};
  ...
  for (const value of values) {
    ...
    switch (value.name) {
      ...
      case 'title':
        ...
        break;
      case 'slug':
        data.slug = ParseValue(value, DataType.string) as string;
        break;
      case 'status':
        data.status = ParseValue(value, DataType.enumeration) as CmsPageStatus;
        break;
      case 'template_layout':
        data.template_layout = ParseValue(value, DataType.string) as string;
        break;
      case 'seo_title':
        data.seo_title = ParseValue(value, DataType.string) as string | null;
        break;
      case 'seo_description':
        data.seo_description = ParseValue(value, DataType.string) as string | null;
        break;
    }
  }
  
  return data as CmsPageModel;
}
...
/**
 * Create index for Slug fields
 */
function createIndexSlug(
  slug: string
): Index {
  const values: Value[] = [
    CreateValue(DataType.string, slug, 'slug'),
  ];
